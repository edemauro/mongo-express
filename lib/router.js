...
var router = function (config) {
  ...
  appRouter.all('*', function (req, res, next) {
    ...
    res.locals.readOnly       = config.options.readOnly || false;
  ...
  // API routes
  // gridfs routes have to come before collection routes
  appRouter.get('/api/db/:database/gridFS/:bucket', mongoMiddleware, routes(config).apiViewBucket);
  
  appRouter.get('/api/index', mongoMiddleware, routes(config).apiIndex);
  appRouter.delete('/api/:database', mongoMiddleware, routes(config).apiDeleteDatabase);
  appRouter.get('/api/db/:database', mongoMiddleware, routes(config).apiViewDatabase);
  appRouter.delete('/api/db/:database/:collection', mongoMiddleware, routes(config).apiDeleteCollection);
  appRouter.get('/api/db/:database/:collection', mongoMiddleware, routes(config).apiViewCollection);
  appRouter.post('/', mongoMiddleware, routes(config).apiAddDatabase);
  appRouter.post('/api/db/:database', mongoMiddleware, routes(config).apiAddCollection);
  appRouter.post('/api/db/:database/:collection', mongoMiddleware, routes(config).apiAddDocument);
  appRouter.put('/api/db/:database/:collection', mongoMiddleware, routes(config).apiRenameCollection);
  appRouter.delete('/api/db/:database/:collection/:document', mongoMiddleware, routes(config).apiDeleteDocument);
  appRouter.get('/api/db/:database/:collection/:document', mongoMiddleware, routes(config).apiViewDocument);
  appRouter.put('/api/db/:database/:collection/:document', mongoMiddleware, routes(config).apiUpdateDocument);
