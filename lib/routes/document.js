...
var routes = function (config) {
  ...
  exp.apiDeleteDocument = function (req, res) {
    const skip            = req.query.skip        || '';
    const key             = req.query.key         || '';
    const value           = req.query.value       || '';
    const type            = req.query.type        || '';
  ...
  exp.apiAddDocument = function (req, res) {
    var doc = req.body.document;
  ...
  exp.apiViewDocument = function (req, res) {
    var ctx = {
      title: (config.options.readOnly ? 'Viewing' : 'Editing') + ' Document: ' + req.document._id,
      editorTheme: config.options.editorTheme,
      docLength: bson.toString(req.document).split(/\r\n|\r|\n/).length,
  ...
  exp.apiUpdateDocument = function (req, res) {
    var doc = req.body.document;
